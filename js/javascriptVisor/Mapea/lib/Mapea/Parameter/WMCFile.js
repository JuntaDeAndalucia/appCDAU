		...
		for (var i=0,len=parameterContexts.length; i<len; i++)
		{
			...
			if (!Mapea.Util.isNullOrEmpty(parameterContext))
			{
				...
				if (urlTitle.length == 1)
				{
					...
					var definedContextsIds = Mapea.Configuration.contexts.split(",");
					var definedContextsUrls = Mapea.Configuration.contextsUrl.split(",");
					var definedContextsTitles = Mapea.Configuration.contextsTitle.split(",");
