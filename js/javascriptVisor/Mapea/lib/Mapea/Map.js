...
//      }
    ...
    */    
     ...
     */    
     addLayer: function (layer, external) {
         ...
         if (external == true) {
            ...
            var existsLayer = false;
            for (var i = 0, ilen = this.externalLayers.length; (i < ilen) && !existsLayer; i++) {
                var externalLayer = this.externalLayers[i];
                existsLayer = (externalLayer.id == layer.id);
            }
            if (!existsLayer) {
    ...
    removeLayer : function(layer, setNewBaseLayer, external) {
        // checks if the layer exists
        var existsLayer = (layer && layer.id && (this.getLayersBy("id", layer.id).length > 0));
        if (existsLayer) {
        ...
        if (external == true) {
            ...
            if (idxLayerToRemove > -1) {
                this.externalLayers.splice(
                        idxLayerToRemove, 1);
            }
        }
        if (existsLayer || (external == true)) {
            this.recalculateMaxZIndex();
        }
    },
    ...
    createLeftPanel : function() {
        ...
        var closeOpenPanel = function(evt) {
            ...
            var jLeftPanel = $(this.leftPanel);
            if (isOpen) {
                var marginLeftCss = jLeftPanel.attr("desplace");
                marginLeftCss = parseInt(marginLeftCss);
                jLeftPanel.attr("open", false).css("margin-left", marginLeftCss)
                    .find("."+activeClassCSS).removeClass(activeClassCSS)
                    .addClass(inactiveClassCSS);
            }
            else {
                jLeftPanel.attr("open", true).css("margin-left", 0)
                    .find("."+inactiveClassCSS).removeClass(inactiveClassCSS)
                    .addClass(activeClassCSS);
    ...
    checkMobileLayerLimit : function(eventLayer) {
        ...
        var mobileLimit = this.nMobileLayers || Mapea.Configuration.numLayerMobile;
    ...
    drawCenter : function(lon, lat, zoom, label, drawPinStr) {
        ...
        if (this.hasAuxLayer()) {
            ...
            // removes previous center features
            ...
            this.auxLayer.events.remove('featureselected');
            this.auxLayer.events.remove('featureunselected');
        }
        ...
        if (label) {
            var labelText = OpenLayers.String.trim(label);
            if (labelText.length > 0) {
                ...
                if (drawPin === true) {
                    ...
                    if (selectAuxFeatureCtrls.length == 0) {
                        ...
                        var showLabelFn = OpenLayers.Function.bind(function() {
                            ...
                            if (mapPopup) {
                                ...
                                var mapPopupCloseFn = OpenLayers.Function.bind(function(evt) {
                                    evt.feature = styledFeature;
                                    evt.feature.layer = this._getAuxLayer();
                                    Mapea.Util.unselectFeature(evt);    
                                    Mapea.Util.removeAllPopups(this);
                                }, this);
                                mapPopup = new Mapea.Popup.FramedCloud("popup_feature",
                                        labelCenter, new OpenLayers.Size(100,50), labelText, null, true,
                                        mapPopupCloseFn);
                                mapPopup.setHeader('<div class="info-header">LABEL</div>');
                    ...
                    var popup = new Mapea.Popup.FramedCloud("popup_feature",
                            this.getCenter(), new OpenLayers.Size(100,50),labelText, null, true);
                    this.addPopup(popup);
                }
            }
        }
